%modified Dijkstra's algorithm, accounting for node failure cost.
%used to find a (node-disjoint) backup path, given primary failed
% 
% INPUTS: 
% adj - adjacency matrix (nxn), s - source node, target - target node
% node_fail_prob - failure prob of each node
% alpha, beta - weight associated with traffic and reliability cost
% node_fail_prob - failure prob. of each node
%
...
function [dist,P]=dij_wProbFail(adj,s,target,alpha,beta,node_fail_prob)
...
while length(Q)>0 % while not empty
    ...
    for v=1:length(adjL{u})   % across all neighbors of u
        v=adjL{u}(v);   
	%Bellman Equation
        alt = dist(u) + alpha*adj(u,v)+...
	      reliability_cost(s,target,u,v,beta,S{u},node_fail_prob);
